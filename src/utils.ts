...
import { InvalidOS, InvalidPropertyType, NonFirefoxFingerprint, UnknownProperty } from './exceptions';
import { fromBrowserforge, generateFingerprint, SUPPORTED_OS } from './fingerprints';
import { publicIP, validIPv4, validIPv6 } from './ip';
import { geoipAllowed, getGeolocation, handleLocales } from './locale';
...
import { sampleWebGL } from './webgl/sample';
...
function updateFonts(config: Record<string, any>, targetOS: string): void {
    const fontsPath = join(__dirname, 'data-files', 'fonts.json');
